...
def trainer(train_data, test_data):
    ...
    clf = RandomForestClassifier(
        n_estimators=1000, max_depth=20, random_state=SEED, n_jobs=-1
    )
...
def create_label(df, perc=[0.5, 0.5]):
    perc = [0.0] + list(np.cumsum(perc))
    label = (
        df.iloc[:, 1:]
        .pct_change(fill_method=None)[1:]
        .apply(lambda x: pd.qcut(x.rank(method="first"), perc, labels=False), axis=1)
    )
    return label
...
for test_year in range(1993, 2020):
    ...
    with open(
        result_folder + "/predictions-" + str(test_year) + ".pickle", "wb"
    ) as handle:
